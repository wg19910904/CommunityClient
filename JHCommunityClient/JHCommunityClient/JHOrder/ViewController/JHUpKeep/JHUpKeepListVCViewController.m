//
//  JHUpKeepListVCViewController.m
//  JHCommunityClient
//
...
@interface JHUpKeepListVCViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>
...
@implementation JHUpKeepListVCViewController
...
- (void)viewDidLoad {
    ...
    Continue_array = @[].mutableCopy;
    completion_array = @[].mutableCopy;
    //创建UISegmentControl
...
-(void)postHttpWithType:(NSString *)type withPage:(NSString * )page{
    NSDictionary * dictionry = @{@"from":@"weixiu",@"type":type, @"page":page};
        ...
        if ([json[@"error"] isEqualToString:@"0"] ) {
            ...
            if ([type isEqualToString:@"0"]) {
                ...
                for(NSDictionary * dic in tempArray){
                    JHUpkeepModel * model = [JHUpkeepModel creatJHUpkeepModelWithDictionary:dic];
...
-(void)creatUIScrollView{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        [myTableView registerClass:[JHUPKeepOrderListCell class] forCellReuseIdentifier:@"cell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == tableView_make) {
        ...
        JHUPKeepOrderListCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        JHUpkeepModel * model = Continue_array[indexPath.row];
        cell.model = model;
        ...
        JHUPKeepOrderListCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        JHUpkeepModel * model = completion_array[indexPath.row];
        cell.model = model;
        //去评价
        cell.btn_pay.tag = indexPath.row;
        [cell.btn_pay addTarget:self action:@selector(clickToPay:) forControlEvents:UIControlEventTouchUpInside];
