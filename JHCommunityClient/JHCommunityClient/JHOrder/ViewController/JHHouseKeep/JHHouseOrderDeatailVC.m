...
#import "JHHouseOrderDetailCellThree.h"
#import "JHHouseOrderDetaiilCellFour.h"
...
@interface JHHouseOrderDeatailVC ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,AVAudioPlayerDelegate>
{
    ...
    JHHouseDetailModel * model_detail;
    JHHouseProgressModel * model_progress;
    BOOL isCall;
...
@implementation JHHouseOrderDeatailVC
...
-(void)postProgressHtttp{
    ...
    [HttpTool postWithAPI:@"client/member/order/log" withParams:dic success:^(id json) {
        ...
        if ([json[@"error"] isEqualToString:@"0"]) {
            HIDE_HUD
            model_progress = [JHHouseProgressModel creatJHHouseProgressModelWithDictionary:json[@"data"][@"log"]];
            if (!isFirst_progress) {
            ...
            for (int i = 0;i<model_progress.modelArray.count;i++) {
                JHHModel * model = model_progress.modelArray[i];
...
-(void)postDetailHttp{
    ...
    [HttpTool postWithAPI:@"client/member/order/detail" withParams:dic success:^(id json) {
        ...
        if ([json[@"error"] isEqualToString:@"0"]) {
            HIDE_HUD
            model_detail = [JHHouseDetailModel creatJHHouseDetailModelWithDictionary:json[@"data"][@"order"]];
            if (!isFirst_detail) {
            ...
            if (([model_detail.order_status intValue] == 0&&[model_detail.staff_id integerValue] == 0)) {
                ...
                if ([model_detail.jiesuan_price integerValue] - [model_detail.danbao_amount integerValue] > 0) {
                   last_money = [model_detail.jiesuan_price floatValue]  - [model_detail.hongbao floatValue];
                    totalMoney = model_detail.jiesuan_price;
                }else{
                    last_money = [model_detail.danbao_amount floatValue] - [model_detail.hongbao floatValue];
                    totalMoney = model_detail.danbao_amount;
                }
               
                money_array = @[[NSString stringWithFormat:@"¥%@",totalMoney],
                                ...
                                [NSString stringWithFormat:@"¥%.2f",last_money]];
            }else if ([model_detail.staff_id integerValue]>0||[model_detail.order_status integerValue] == 3||[model_detail.order_status integerValue] == 5||([model_detail.order_status integerValue] == 1&&[model_detail.staff_id integerValue] > 0)||[model_detail.order_status integerValue] == -1){
                array = @[@"",@"",[NSString stringWithFormat:NSLocalizedString(@"订单ID:%@", nil),model_detail.order_id],
                          [NSString stringWithFormat:NSLocalizedString(@"联系人:%@", nil),model_detail.contact],
                          [NSString stringWithFormat:NSLocalizedString(@"联系电话:%@", nil),model_detail.mobile],
                           [NSString stringWithFormat:NSLocalizedString(@"服务类型:%@", nil),model_detail.cate_title],
...
-(void)creatUIScrollView{
    ...
    for (int i = 0; i<2; i++) {
        ...
        if(i == 0){
            ...
            [myTableView_order registerClass:[JHHouseOrderDetailCellOne class] forCellReuseIdentifier:@"cell"];
            [myTableView_order registerClass:[JHHouseOrderDetailCellTwo class] forCellReuseIdentifier:@"cell2"];
            ...
            [myTableView_detail registerClass:[JHHouseOrderDetailCellThree class] forCellReuseIdentifier:@"cell3"];
            [myTableView_detail registerClass:[JHHouseOrderDetaiilCellFour class] forCellReuseIdentifier:@"cell4"];
            [myTableView_detail registerClass:[JHHouseOrderDetailCellFive class] forCellReuseIdentifier:@"cell5"];
            [myTableView_detail registerClass:[JHHouseOrderDetailCellSix class] forCellReuseIdentifier:@"cell6"];
        }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == myTableView_order) {
        ...
        if (indexPath.row == 0) {
            ...
            return 15;
        }else if (indexPath.row == 14 && [model_detail.order_status integerValue] == 0 && [model_detail.staff_id integerValue] == 0){
            return 15;
        ...
        }else if (indexPath.row == 19 && [model_detail.hongbao integerValue] == 0){
            return 0;
        }
        else{
            return 40;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == myTableView_order) {
        if (indexPath.row == 0) {
            JHHouseOrderDetailCellOne * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
            ...
            JHHouseOrderDetailCellTwo * cell = [tableView dequeueReusableCellWithIdentifier:@"cell2" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
//            [btn_call addTarget:self action:@selector(clickToCall) forControlEvents:UIControlEventTouchUpInside];
        ...
        if (indexPath.row == 0) {
            JHHouseOrderDetailCellThree * cell = [tableView dequeueReusableCellWithIdentifier:@"cell3" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            JHHouseOrderDetaiilCellFour * cell = [tableView dequeueReusableCellWithIdentifier:@"cell4" forIndexPath:indexPath];
            cell.height = height;
            cell.myLabel.text = [NSString stringWithFormat:NSLocalizedString(@"服务要求:%@", nil),model_detail.intro];
            return cell;
        }else if (indexPath.row == 10){
            JHHouseOrderDetailCellFive * cell = [tableView dequeueReusableCellWithIdentifier:@"cell5" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            JHHouseOrderDetailCellSix * cell = [tableView dequeueReusableCellWithIdentifier:@"cell6" forIndexPath:indexPath];
            ...
            if(indexPath.row == 16&&[model_detail.order_status integerValue] == 0&&[model_detail.staff_id integerValue] == 0){
                if([model_detail.order_status intValue] == 0 && [model_detail.pay_status intValue] == 0){
    ...
    if ([sender.titleLabel.text isEqualToString:NSLocalizedString(@"去支付", nil)]) {
        ...
        [vc setPaySuccessBlock:^(BOOL success, NSString *msg) {
            if (success) {
                ...
                [self showMsg:NSLocalizedString(@"支付成功",nil)];
            }
        }];
