...
@interface JHGroupListController ()<UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHGroupListController
...
-(void)postHttpWithType:(NSString *)type withPage:(NSString * )page{
    NSDictionary * dictionry = @{@"from":@"tuan",@"type":type, @"page":page};
        ...
        if ([json[@"error"] isEqualToString:@"0"] ) {
            if ([type isEqualToString:@"0"]) {
                ...
                for(NSDictionary * dic in tempArray){
                    JHGroupModel * model = [JHGroupModel creatJHGroupModelWithDictionary:dic];
...
-(void)creatUIScrollView{
    ...
    for(int i = 0; i < 2;i++){
        ...
        [tableView registerClass:[JHGroupListTableViewCell class] forCellReuseIdentifier:@"cell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (Continue_array.count == 0) {
        ...
        JHGroupListTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        JHGroupModel * model = Continue_array[indexPath.row];
        cell.model = model;
        ...
        JHGroupListTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
        JHGroupModel * model = completion_array[indexPath.row];
        cell.model = model;
        cell.btn.tag = indexPath.row;
        [cell.btn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
        //cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
-(void)btnClick:(UIButton *)sender{
    ...
    if ([sender.titleLabel.text isEqualToString:NSLocalizedString(@"去支付", nil)]) {
       ...
       [HttpTool postWithAPI:@"client/tuan/order/cancel" withParams:@{@"order_id":model.order_id} success:^(id json) {
           if ([json[@"error"] isEqualToString:@"0"]) {
               [Continue_array removeObjectAtIndex:sender.tag];
...
-(void)clickToCancel:(UIButton *)sender{
    NSLog(@"点击的是取消订单的方法");
    SHOW_HUD
    JHGroupModel * model = Continue_array[sender.tag];
    [self cancelOrderWithOrder_id:model.order_id withRow:sender.tag];
}
