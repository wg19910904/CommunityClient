//
//  JHBuyOrderDetailViewController.m
//  JHCommunityClient
//
...
#import "JHBuyOrderDetailCell.h"
#import "JHBuyOrderDetailCellOne.h"
#import "JHBuyOrderDetailCellTwo.h"
#import "JHBuyOrderDeatilCellThree.h"
#import <MJRefresh.h>
...
@interface JHBuyOrderDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,MAMapViewDelegate,AVAudioPlayerDelegate>
{
    ...
    NSMutableArray * lieArray;//填充假数据的数组
    float height;//自适应的高度
    UIImageView * imageView;//指向音浪的
    ...
    float lng;//当前的外卖的经度
    NSString * pay_status ;
    NSString * paotui_price;
    BOOL _isBuyVc;
}
@end

@implementation JHBuyOrderDetailViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    //@[@"",@"",NSLocalizedString(@"跑腿类别:帮我买", nil),NSLocalizedString(@"订单ID:1001", nil),NSLocalizedString(@"联系人:哈哈哈", nil),NSLocalizedString(@"联系电话:13888888888", nil),NSLocalizedString(@"收货地址:合肥望江路与合作化路南路交口55号", nil),NSLocalizedString(@"收货时间:2016-3-2 20:00", nil)];
    ...
    [HttpTool postWithAPI:@"client/member/order/detail" withParams:dic success:^(id json) {
        ...
        if ([json[@"error"] isEqualToString:@"0"]) {
            ...
            if(([model_detail.order_status intValue] == 0 && [model_detail.pay_status intValue] == 0)||([model_detail.order_status intValue] == -1&&[model_detail.pay_status intValue] == 0)){
                ...
                num = 11;
            }else if ([model_detail.order_status intValue] == 3||[model_detail.order_status intValue] == 4||[model_detail.order_status intValue] == 5){
                num = 20;
            }else if([model_detail.order_status intValue] == 8){
                num = 20;
            }
            if ([model_detail.pay_status intValue] == 0&&[model_detail.order_status integerValue] != 5) {
                pay_status = NSLocalizedString(@"担保金额(未支付)", nil);
                paotui_price = NSLocalizedString(@"跑腿费(未支付)", nil);
            }else if ([model_detail.pay_status intValue] == 0&&[model_detail.order_status integerValue] == 5){
                pay_status = NSLocalizedString(@"担保金额(已支付)", nil);
                paotui_price = NSLocalizedString(@"跑腿费(已支付)", nil);
            }
            else if([model_detail.pay_status intValue] == 1){
                pay_status = NSLocalizedString(@"担保金额(已支付)", nil);
                paotui_price = NSLocalizedString(@"跑腿费(已支付)", nil);
            }
            if (num < 19) {
                lieArray =  [NSMutableArray arrayWithObjects:@"",@"",
                            [NSString stringWithFormat:NSLocalizedString(@"跑腿类别:%@", nil),type],
                            [NSString stringWithFormat:NSLocalizedString(@"订单ID:%@", nil),model_detail.order_id],[NSString stringWithFormat:NSLocalizedString(@"联系人:%@", nil),model_detail.contact],
                            ...
                            [NSString stringWithFormat:NSLocalizedString(@"联系人:%@", nil),model_detail.o_name? model_detail.o_name : @""],
                            [NSString stringWithFormat:NSLocalizedString(@"联系电话:%@", nil),model_detail.o_mobile? model_detail.o_mobile : @""],
                            @"",@"",
                            pay_status,paotui_price,nil];
                array = @[[NSString stringWithFormat:@"¥%@",model_detail.danbao_amount],
                          [NSString stringWithFormat:@"¥%@",model_detail.paotui_amount],@"-¥2",@"¥38",@"¥48"];
            }else{
                lieArray =  [NSMutableArray arrayWithObjects:@"",@"",
                            [NSString stringWithFormat:NSLocalizedString(@"跑腿类别:%@", nil),type],
                            ...
                            [NSString stringWithFormat:NSLocalizedString(@"联系电话:%@", nil),model_detail.o_mobile? model_detail.o_mobile : @""],
                            @"",@"",
                            NSLocalizedString(@"总计", nil),NSLocalizedString(@"预估金额", nil),[model_detail.hongbao integerValue] == 0 ? @"" : NSLocalizedString(@"红包抵扣", nil),NSLocalizedString(@"实际支付", nil),NSLocalizedString(@"跑腿费", nil),nil];
                float a = [model_detail .paotui_amount floatValue] + [model_detail.hongbao floatValue];
                array = @[[NSString stringWithFormat:@"¥%.2f",a],
                          [NSString stringWithFormat:@"%@",[model_detail.expected_price floatValue]==0?NSLocalizedString(@"", nil):[@"¥" stringByAppendingString:model_detail.expected_price]],[model_detail.hongbao floatValue] == 0 ? @"" :model_detail.hongbao,[NSString stringWithFormat:@"¥ %@",model_detail.paotui_amount]];
...
-(void)creatUIScrollView{
    ...
    for (int i = 0; i<2; i++) {
        ...
        if(i == 0){
            ...
            [myTableView_order registerClass:[JHBuyOrderDetailCell class] forCellReuseIdentifier:@"cell"];
            [myTableView_order registerClass:[JHBuyOrderDetailCellOne class] forCellReuseIdentifier:@"cell2"];
            ...
            [myTableView_detail registerClass:[JHBuyOrderDetailCellTwo class] forCellReuseIdentifier:@"cell3"];
            [myTableView_detail registerClass:[JHBuyOrderDeatilCellThree class] forCellReuseIdentifier:@"cell4"];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == myTableView_order) {
        ...
        if (indexPath.row == 0) {
            ...
            if (indexPath.row == 9) {
                if (num > 11||[model_detail.danbao_amount integerValue] == 0) {
                    return 0;
                }else{
                    return 40;
                }
            ...
            }else if(indexPath.row == 16 && [model_detail.order_status integerValue] < 8 && [model_detail.danbao_amount integerValue] == 0){
                return 0;
            }else if (indexPath.row == 18 && [model_detail.hongbao integerValue] == 0){
                return 0;
            }
            else{
                if(indexPath.row ==17 ){
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == myTableView_order) {
        if (indexPath.row == 0) {
            JHBuyOrderDetailCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
            ...
            JHBuyOrderDetailCellOne * cell = [tableView dequeueReusableCellWithIdentifier:@"cell2" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
//            [btn_call addTarget:self action:@selector(clickToCall) forControlEvents:UIControlEventTouchUpInside];
        ...
        if (indexPath.row == 0) {
            JHBuyOrderDetailCellTwo * cell = [tableView dequeueReusableCellWithIdentifier:@"cell3" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            JHBuyOrderDeatilCellThree * cell = [tableView dequeueReusableCellWithIdentifier:@"cell4" forIndexPath:indexPath];
          ...
          static NSString * str_identifier = @"ce";
            UITableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:str_identifier];
            cell.backgroundColor = [UIColor whiteColor];
            cell.textLabel.text = @"";
            cell.detailTextLabel.text = @"";
            if (cell == nil) {
                cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:str_identifier];
            }
             if(indexPath.row == 9){
                 if (num > 11||[model_detail.danbao_amount integerValue] == 0) {
                     
                 }else{
                cell.textLabel.text = pay_status;
                ...
                cell.detailTextLabel.font =[UIFont systemFontOfSize:13];
                 ...
                 if (num > 11) {
                     ...
                     cell.textLabel.text = paotui_price;
                     cell.textLabel.textColor = [UIColor colorWithWhite:0.4 alpha:1];
                     cell.textLabel.font = [UIFont systemFontOfSize:15];
                     cell.detailTextLabel.text = [NSString stringWithFormat:@"-¥ %@",model_detail.paotui_amount];
                     cell.detailTextLabel.textColor = [UIColor orangeColor];
                     cell.detailTextLabel.font =[UIFont systemFontOfSize:13];
                 }
             }
                 ...
                 cell.textLabel.text = lieArray[indexPath.row];
                 NSLog(@"%@%ld",lieArray[indexPath.row],indexPath.row);
                 cell.textLabel.textColor = [UIColor colorWithWhite:0.4 alpha:1];
                 cell.textLabel.font = [UIFont systemFontOfSize:15];
                 if (indexPath.row == 3) {
                 ...
                 if (indexPath.row > 15) {
                     if (indexPath.row == 16&&[model_detail.danbao_amount integerValue] == 0&&[model_detail.order_status integerValue] < 8) {
                         cell.textLabel.text = @"";
                     }else
                     {
                     cell.detailTextLabel.text = array[indexPath.row-16];
                     cell.detailTextLabel.textColor = [UIColor orangeColor];
                     cell.detailTextLabel.font =[UIFont systemFontOfSize:13];
                     }
                 }
...
-(void)clickToMore:(UIButton *)sender{
    if ([sender.titleLabel.text isEqualToString:NSLocalizedString(@"去支付", nil)]) {
        ...
        vc.amount = [NSString stringWithFormat:@"%ld", [model_progress.danbao_amount integerValue] + [model_progress.paotui_amount integerValue]];
        vc.isDetailVC = YES;
...
-(void)judgeBtnState:(UIButton *)sender{
   if ([model_progress.order_status integerValue] == -1){
   ...
   else if([model_progress.order_status integerValue] != 5 && [model_progress.pay_status integerValue] == 0){
        [sender setTitle:NSLocalizedString(@"去支付", nil) forState:UIControlStateNormal];
        sender.backgroundColor = [UIColor orangeColor];
        
    }else if ([model_progress.order_status integerValue] == 0){
    ...
    else if ([model_progress.order_status integerValue] == 1 && [model_progress.staff_id integerValue] > 0){
        [sender setTitle:NSLocalizedString(@"等待服务", nil) forState:UIControlStateNormal];
        sender.backgroundColor = [UIColor colorWithRed:225/255.0 green:230/255.0 blue:230/255.0 alpha:1];
    }
        ...
        [sender setTitle:NSLocalizedString(@"等待服务", nil) forState:UIControlStateNormal];
        sender.backgroundColor = [UIColor colorWithRed:225/255.0 green:230/255.0 blue:230/255.0 alpha:1];
    }else if([model_progress.order_status intValue] == 5||[model_progress.order_status integerValue] == 4){
