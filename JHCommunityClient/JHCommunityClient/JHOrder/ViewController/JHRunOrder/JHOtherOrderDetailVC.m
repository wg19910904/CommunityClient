...
#import <MJRefresh.h>
#import "JHOtherOrderDetailCell.h"
#import "JHOtherOrderDetailCellOne.h"
#import "JHOtherOrderDetailCellTwo.h"
#import "JHOtherOrderDetailCellThree.h"
...
@interface JHOtherOrderDetailVC ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,MAMapViewDelegate,AVAudioPlayerDelegate>
...
@implementation JHOtherOrderDetailVC
...
-(void)postDetailHttp{
    ...
    [HttpTool postWithAPI:@"client/member/order/detail" withParams:dic success:^(id json) {
        ...
        if ([json[@"error"] isEqualToString:@"0"]) {
            ...
            array = [NSMutableArray arrayWithObjects:@"",@"",NSLocalizedString(@"跑腿类别:其他", nil),
...
-(void)creatUIScrollView{
    ...
    for (int i = 0; i<2; i++) {
        ...
        if(i == 0){
            ...
            [myTableView_order registerClass:[JHOtherOrderDetailCell class] forCellReuseIdentifier:@"cell"];
            [myTableView_order registerClass:[JHOtherOrderDetailCellOne class] forCellReuseIdentifier:@"cell2"];
            ...
            [myTableView_detail registerClass:[JHOtherOrderDetailCellTwo class] forCellReuseIdentifier:@"cell3"];
            [myTableView_detail registerClass:[JHOtherOrderDetailCellThree class] forCellReuseIdentifier:@"cell4"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == myTableView_order) {
        if (indexPath.row == 0) {
            JHOtherOrderDetailCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
            ...
            JHOtherOrderDetailCellOne * cell = [tableView dequeueReusableCellWithIdentifier:@"cell2" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
//            [btn_call addTarget:self action:@selector(clickToCall) forControlEvents:UIControlEventTouchUpInside];
        ...
        if (indexPath.row == 0) {
            JHOtherOrderDetailCellTwo * cell = [tableView dequeueReusableCellWithIdentifier:@"cell3" forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            JHOtherOrderDetailCellThree * cell = [tableView dequeueReusableCellWithIdentifier:@"cell4" forIndexPath:indexPath];
...
-(void)cancelOrderWithOrder_id:(NSString *)order_id {
    [HttpTool postWithAPI:@"client/order/cancel" withParams:@{@"order_id":order_id} success:^(id json) {
        NSLog(@"%@",json);
        if ([json[@"error"] isEqualToString:@"message"]) {
            HIDE_HUD
            //刷新数据
        ...
        [self creatUIAlertControlWithMessage:NSLocalizedString(@"连接服务器出错,稍后请重试", nil)];
    }];
}
