...
@implementation JHUpKeepProgressModel
+(JHUpKeepProgressModel *)creatJHUpKeepProgressModelWithDictionary:(NSDictionary *)dic{
    return [[JHUpKeepProgressModel alloc]initWithDictionary:dic];
}
-(id)initWithDictionary:(NSDictionary *)dic{
    if (self == [super init]) {
        ...
        self.chajia = [NSString stringWithFormat:@"%.2f",[dic[@"jiesuan_price"] floatValue] - [dic[@"amount"] floatValue]-self.hongbao.floatValue];
        NSArray * tempArray = dic[@"log"];
        for (NSDictionary * dictionary in tempArray) {
            JHUModel * model = [JHUModel creatJHUModelWithDictionary:dictionary];
...
@end
@implementation JHUModel
+(JHUModel * )creatJHUModelWithDictionary:(NSDictionary *)dic{
    return [[JHUModel alloc]initWithDictionary:dic];
}
-(id)initWithDictionary:(NSDictionary * )dic{
    if (self == [super init]) {
        self.dateline = [self returnTimeWithDateLine:dic[@"dateline"]];
