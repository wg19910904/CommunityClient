...
@implementation TakeawayTableViewCellOne
{
...
-(void)setModel:(JHTakeyawayModel*)model{
    _model = model;
    //创建头部的分割线
    if(view_LineOne == nil){
    ...
    imageView_type.image = [UIImage imageNamed:@"wai"];
    //创建显示订单名的label
    if(label_name == nil){
        label_name = [[UILabel alloc]init];
        label_name.frame = FRAME(40, 10, WIDTH/2, 20);
    ...
    label_name.text = model.product_name;
    //创建显示订单的状态的(待支付/已支付)
    if(label_state == nil){
    ...
    label_count.text = [NSString stringWithFormat:NSLocalizedString(@"%@份", nil),model.product_number];
    //创建显示总的钱数的label
    if(label_totalMoney == nil){
        label_totalMoney = [[UILabel alloc]init];
        label_totalMoney.frame = FRAME(80, 84, 200, 20);
    ...
    label_totalMoney.text = [NSString stringWithFormat:NSLocalizedString(@"¥ %@", nil),model.amount];
    //创建显示下单时间的label
    if(label_time == nil){
    ...
    NSMutableAttributedString * attributed = [[NSMutableAttributedString alloc]initWithString:label_time.text];
    NSMutableParagraphStyle * paragrap = [[NSMutableParagraphStyle alloc]init];
    [paragrap setLineSpacing:3];
    [attributed addAttribute:NSParagraphStyleAttributeName value:paragrap range:NSMakeRange(0, label_time.text.length)];
    label_time.attributedText = attributed;
    //创建显示去支付还是去评价还是已评价的按钮
    if (self.btn == nil) {
        self.btn =[JHOrderBtn buttonWithType:UIButtonTypeCustom];
    ...
    if([model.order_status intValue] == 0 && [model.pay_status intValue] == 0&&[model.online_pay isEqualToString:@"1"]){
        ...
        self.btn.backgroundColor = THEME_COLOR;
    }else if(([model.order_status integerValue] == 1||[model.order_status integerValue] == 2)&&[model.pei_type integerValue] != 3){
        [self.btn setTitle:NSLocalizedString(@"等待配送", nil) forState:UIControlStateNormal];
        if (model.pei_type.integerValue == 4) {
            [self.btn setTitle:NSLocalizedString(@"等待出餐", nil) forState:UIControlStateNormal];
        }
        self.btn.backgroundColor = [UIColor colorWithRed:225/255.0 green:230/255.0 blue:230/255.0 alpha:1];
    }else if(([model.order_status integerValue] == 1||[model.order_status integerValue] == 2)&&[model.pei_type integerValue] == 3){
        [self.btn setTitle:NSLocalizedString(@"待自提", nil) forState:UIControlStateNormal];
        self.btn.backgroundColor = [UIColor colorWithRed:225/255.0 green:230/255.0 blue:230/255.0 alpha:1];
    }
    else if ([model.order_status integerValue]== 4||[model.order_status intValue] == 3){
        [self.btn setTitle:NSLocalizedString(@"确认完成", nil) forState:UIControlStateNormal];
         self.btn.backgroundColor = THEME_COLOR;
    }
        ...
        [self.btn setTitle:NSLocalizedString(@"已评价", nil) forState:UIControlStateNormal];
         self.btn.backgroundColor = [UIColor colorWithRed:225/255.0 green:230/255.0 blue:230/255.0 alpha:1];
;
    }else if ([model.staff_id integerValue] > 0  && [model.order_status intValue] == 1){
        [self.btn setTitle:NSLocalizedString(@"等待配送", nil) forState:UIControlStateNormal];
        if (model.pei_type.integerValue == 4) {
            [self.btn setTitle:NSLocalizedString(@"等待出餐", nil) forState:UIControlStateNormal];
        }
    ...
    if(self.cancelBtn == nil){
        ...
        self.cancelBtn.frame = CGRectMake(WIDTH - 160, 125, 70, 30);
