//
//  TakeawayOrderProgressCellTwo.m
//  JHCommunityClient
//
...
@implementation TakeawayOrderProgressCellTwo
{
...
-(void)setModel:(JHTakewayProgressModel *)model{
    _model = model;
    JHOtherModel * otherModel = self.infoArray[self.indexPath.row - 2];
    //创建头像
    ...
    if (self.indexPath.row == 2 && self.infoArray.count == 1) {
        self.label_praogressOne.hidden = YES;
        self.label_praogressTwo.hidden = YES;
    }else if (self.indexPath.row == 2&&self.infoArray.count > 1) {
        self.label_praogressOne.hidden = YES;
    }else if (self.indexPath.row == self.infoArray.count + 1&&self.infoArray.count > 1){
        self.label_praogressTwo.hidden = YES;
    }
    //创建显示订单状态的label
    if (label_state == nil) {
