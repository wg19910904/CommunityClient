...
@implementation JHHouseOrderDetailCellThree
{
    UILabel * label_state;//显示订单状态的
    UIView * label_line;//创建分割线
}

-(void)setModel:(JHHouseDetailModel *)model{
    _model = model;
    if (label_state == nil) {
        label_state = [[UILabel alloc]init];
        label_state.frame = FRAME(15, 5, WIDTH/2+50, 50);
...
//    if ([model.order_status integerValue] == -1 && [model.pay_status integerValue] == 1) {
//        self.btn.hidden = NO;
//    }else{
//        self.btn.hidden = YES;
//    }
