...
-(void)setModel:(JHUpkeepModel *)model{
    _model = model;
    if(label_line == nil){
        //创建第一根分割线
  ...
  if ([model.order_status intValue] == -1){
    ...
    }else if([model.pay_status intValue] == 0&&[model.order_status integerValue] != 5){
    ...
    if([model.pay_status intValue] == 0 && [model.order_status integerValue] != 5 && [model.order_status intValue] != -1){
