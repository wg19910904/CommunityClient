...
@implementation JHRunPetTableViewCell
{
...
-(void)setModel:(JHRunModel *)model{
    ...
    label_type.text = NSLocalizedString(@"宠物照顾", nil);
    //创建去支付还剩多少时间的
    if(label_time == nil){
        label_time = [[UILabel alloc]init];
        label_time.frame = FRAME(90, 10, 100, 20);
    ...
    if(imageViewOne == nil){
        ...
        imageViewOne.image = [UIImage imageNamed:@"petHead"];
    ...
    label_shop.text = [NSString stringWithFormat:NSLocalizedString(@"宠物照顾描述:%@", nil),model.intro];
    //显示地理位置的
