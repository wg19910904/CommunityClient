...
@implementation JHRunOtherTableViewCell
{
...
-(void)setModel:(JHRunModel *)model{
    ...
    label_type.text = NSLocalizedString(@"其他", nil);
    //创建去支付还剩多少时间的
    if(label_time == nil){
        label_time = [[UILabel alloc]init];
        label_time.frame = FRAME(50, 10, 100, 20);
    ...
    if(imageViewOne == nil){
        ...
        imageViewOne.image = [UIImage imageNamed:@"daishiyong"];
    ...
    label_shop.text = [NSString stringWithFormat:NSLocalizedString(@"服务要求:%@", nil),model.intro];
    //显示地理位置的
    ...
    label_address.text = [NSString stringWithFormat:NSLocalizedString(@"服务地址:\n%@", nil),model.o_addr];
    //显示订单号的
    if(label_order == nil){
