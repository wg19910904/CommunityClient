...
@implementation JHSeatTableViewCell
{
...
-(void)setModel:(JHRunModel *)model{
    ...
    label_type.text = NSLocalizedString(@"餐馆占座", nil);
    //创建去支付还剩多少时间的
    if(label_time == nil){
    ...
    if(imageViewOne == nil){
        ...
        imageViewOne.image = [UIImage imageNamed:@"seatImage"];
    ...
    label_shop.text = [NSString stringWithFormat:NSLocalizedString(@"服务描述:%@", nil),model.intro];
    //显示地理位置的
