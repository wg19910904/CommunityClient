...
@implementation JHRunTableViewCellOther
{
...
-(void)setModel:(JHRunModel *)model{
    ...
    label_type.text = NSLocalizedString(@"代排队", nil);
    //创建去支付还剩多少时间的
    if(label_time == nil){
    ...
    if(imageViewOne == nil){
        ...
        imageViewOne.frame = FRAME(17.5, 50, 15, 20);
        [self addSubview:imageViewOne];
        imageViewOne.image = [UIImage imageNamed:@"zuobaio2"];
        //imageViewOne.backgroundColor = [UIColor redColor];
        imageViewTwo = [[UIImageView alloc]init];
        imageViewTwo.frame = FRAME(15, 115, 20, 20);
        imageViewTwo.image = [UIImage imageNamed:@"phone03"];
    ...
    if(label_shop == nil){
        ...
        label_shop.frame = FRAME(40, 110,  WIDTH - 70, 20);
    ...
    NSMutableString *phone = model.mobile.mutableCopy;
    if (!phone || phone.length == 0) {
        label_shop.text = NSLocalizedString(@"联系电话暂时为空", nil);
    }else{
        NSInteger index = (phone.length - 4)/2;
        [phone replaceCharactersInRange:NSMakeRange(index, 4) withString:@"****"];
        label_shop.text = [NSString stringWithFormat:NSLocalizedString(@"联系电话:%@", nil),phone];
    }
    //显示地理位置的
    if(label_address == nil){
        label_address = [[UILabel alloc]init];
        label_address.frame = FRAME(40, 46, WIDTH - 70, 60);
    ...
    label_address.text = [NSString stringWithFormat:NSLocalizedString(@"排队地址:\n%@", nil),model.o_addr];
    //显示订单号的
    if(label_order == nil){
