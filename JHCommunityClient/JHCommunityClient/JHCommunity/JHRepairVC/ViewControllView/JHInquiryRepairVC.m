...
@interface JHInquiryRepairVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHInquiryRepairVC
...
- (void)viewDidLoad {
    ...
    _api = @"client/xiaoqu/baoxiu/items";
    _alertApi = @"client/xiaoqu/baoxiu/tixing";
    _deleteApi = @"client/xiaoqu/baoxiu/delete";
    _cancelApi = @"client/xiaoqu/baoxiu/cancel";
    [self loadNewData];
}
#pragma mark--===创建查询表视图
    ...
    [CommunityHttpTool postWithAPI:_api withParams:dic success:^(id json) {
        ...
        if([json[@"error"] isEqualToString:@"0"]){
            ...
            for(NSDictionary *dic in items){
                JHInquiryRepairModel *model = [[JHInquiryRepairModel alloc] init];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    JHInquiryRepairCell *cell = [[JHInquiryRepairCell alloc] init];
    JHInquiryRepairModel *model= _dataSource[indexPath.section];
    cell.inquiryModel = model;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *identifier = @"inquiryRepairCell";
    JHInquiryRepairCell *cell = [_inquiryTableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[JHInquiryRepairCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
...
- (void)clickAlertButton:(UIButton *)sender{
    NSLog(@"提醒");
    SHOW_HUD
    NSDictionary *dic = @{@"baoxiu_id":[_dataSource[sender.tag - 1] baoxiu_id]};
...
- (void)clickCancelButton:(UIButton *)sender{
    NSLog(@"撤销");
    SHOW_HUD
    NSDictionary *dic = @{@"baoxiu_id":[_dataSource[sender.tag - 2] baoxiu_id]};
...
- (void)clickDeleteButton:(UIButton *)sender{
    NSLog(@"删除");
    SHOW_HUD
    NSDictionary *dic = @{@"baoxiu_id":[_dataSource[sender.tag - 3] baoxiu_id]};
