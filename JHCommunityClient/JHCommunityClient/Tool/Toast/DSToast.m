...
static CFTimeInterval const kDefaultForwardAnimationDuration = 0.6;
static CFTimeInterval const kDefaultBackwardAnimationDuration = 0.6;
static CFTimeInterval const kDefaultWaitAnimationDuration = 0.6;
...
- (void)showInView:(UIView *)view
{
    ...
    self.center = point;
    if (myView == nil) {
        myView = [[UIView alloc]initWithFrame:view.bounds];
...
- (void)showInView:(UIView *)view showType:(DSToastShowType)type withBlock:(void(^)(void))block
{
    [self addAnimationGroup];
    CGPoint point = view.center;
    switch (type) {
    ...
    if (myView == nil) {
        ...
        swipe.direction = swipe.direction = UISwipeGestureRecognizerDirectionUp|
        UISwipeGestureRecognizerDirectionDown;
;
        [myView addGestureRecognizer:swipe];
        [self setMyBlock:^(void){
            block();
        }];
...
-(void)swipeToRemove:(UISwipeGestureRecognizer *)swipe{
    [self removeFromSuperview];
    [myView removeFromSuperview];
    myView = nil;
    if (self.myBlock) {
