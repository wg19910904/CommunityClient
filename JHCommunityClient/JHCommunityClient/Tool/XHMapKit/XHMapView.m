...
@implementation XHMapView
{
    ...
    AMapWalkingRouteSearchRequest *request1;
    AMapWalkingRouteSearchRequest *request2;
}
- (instancetype)init{
    self = [super init];
    if (self) {
...
- (MAMapView *)gaodeMap{
    if (_gaodeMap == nil) {
        ...
        _gaodeMap.userTrackingMode = MAUserTrackingModeFollow;
    }
    return _gaodeMap;
}
...
- (void)mapView:(MAMapView *)mapView didUpdateUserLocation:(MAUserLocation *)userLocation updatingLocation:(BOOL)updatingLocation{
    ...
    self.user_lng = userLocation.coordinate.latitude;
    if (self.XHDelegate&&[self.XHDelegate respondsToSelector:@selector(xhMapView:didUpdateUserLocation:updatingLocation:)]) {
        [self.XHDelegate xhMapView:mapView didUpdateUserLocation:userLocation updatingLocation:updatingLocation];
    }
}
/** *@brief 地图移动结束后调用此接口
...
- (void)mapView:(MAMapView *)mapView mapDidMoveByUser:(BOOL)wasUserAction{
    if (self.XHDelegate && [self.XHDelegate respondsToSelector:@selector(xhMapView:mapDidMoveByUser:)]) {
        [self.XHDelegate xhMapView:mapView mapDidMoveByUser:wasUserAction];
    }
}
/** *
 @brief 地图缩放结束后调用此接口
...
- (void)mapView:(MAMapView *)mapView mapDidZoomByUser:(BOOL)wasUserAction{
    if (self.XHDelegate && [self.XHDelegate respondsToSelector:@selector(xhMapView:mapDidZoomByUser:)]) {
        [self.XHDelegate xhMapView:mapView mapDidZoomByUser:wasUserAction];
    }
}
//标注大头针会回调的方法
...
- (void)setCenterWithCurrentLocation{
    ...
    if ([XHMapKitManager shareManager].is_International) {
        ...
        //国内客户
        [_gaodeMap setCenterCoordinate:CLLocationCoordinate2DMake([XHMapKitManager shareManager].lat, [XHMapKitManager shareManager].lng)];
    }
}
...
//    CLLocationCoordinate2D pei = [self changeBaiDuToGaodeWithBaidu:peiCoordinate];
    NSString *dis = [self getDistanceWithPoint1:customCoordinate point2:peiCoordinate];
    NSString *sub = [NSString stringWithFormat:NSLocalizedString(@"配送员距您还有%@", nil),dis];
    [self changePraviteWithPeiCoordinate:peiCoordinate subT:sub mainT:NSLocalizedString(@"送货中...", nil)];
...
- (void)getGaodeMapNavPointsWithOrigin_lat:(double)origin_lat
    ...
    if (_search1 == nil) {
        ...
        request1 = [[AMapWalkingRouteSearchRequest alloc] init];
        ...
        [_search1 AMapWalkingRouteSearch:request1];
    }else{
        _search2 = [[AMapSearchAPI alloc] init];
        _search2.delegate = self;
        request2 = [[AMapWalkingRouteSearchRequest alloc] init];
        ...
        request2.destination = [AMapGeoPoint locationWithLatitude:destination_lat
                                                        longitude:destination_lng];
        [_search2 AMapWalkingRouteSearch:request2];
    }
}
...
- (void)getGMSMapNaviPointsWithOrigin_lat:(double)origin_lat
    ...
    _gmsMap.camera = [GMSCameraPosition cameraWithLatitude:[XHMapKitManager shareManager].lat
                                                 longitude:[XHMapKitManager shareManager].lng
                                                      zoom:14];
    NSString *url=[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/directions/json?origin=%f,%f&destination=%f,%f&sensor=false&mode=%@",origin_lat,origin_lng,destination_lat,destination_lng,@"diving"];
...
#pragma mark - 屏幕对应点的坐标
- (CLLocationCoordinate2D)coordinateForPoint:(CGPoint)point{
    if ([XHMapKitManager shareManager].is_International) {
        //谷歌地图
        return [_gmsMap.projection coordinateForPoint:point];
    }else{
        //高德地图
        return [_gaodeMap convertPoint:point toCoordinateFromView:self];
    }
}

#pragma mark - 移除所有的大头针
