...
-(void)initData{
    ...
    [rightItem setTitleColor:TEXT_COLOR forState:UIControlStateNormal];
...
-(void)creatUISearch{
    if (searchTextField == nil) {
        ...
        searchTextField.layer.borderColor = LINE_COLOR.CGColor;
...
-(void)creatAroundTableView{
    aroundTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, (HEIGHT - 64)/2+64, WIDTH, (HEIGHT - 64)/2) style:UITableViewStylePlain];
...
-(void)creatKeyTableView{
    if (keyTableView == nil) {
        ...
        keyTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, origin_y, WIDTH, HEIGHT-origin_y) style:UITableViewStylePlain];
...
-(void)creatSearchWithlatiude:(float)lat withLongitude:(float)log{
    ...
    request.types = NSLocalizedString(@"餐饮服务|购物服务|生活服务|住宿服务|商务住宅|公司企业", nil);
    request.sortrule = 0;
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response
{
    ...
    for (AMapPOI * poi in response.pois) {
        ...
        model.city = poi.city;
        [XHMapKitManager shareManager].currentCity =poi.city;
