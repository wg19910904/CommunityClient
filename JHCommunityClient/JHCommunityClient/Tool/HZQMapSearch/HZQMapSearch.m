...
@interface HZQMapSearch ()<MAMapViewDelegate,AMapSearchDelegate,UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
{
...
-(void)initData{
    ...
    //检测文本框发生文本发生改变的通知
    [NoticeCenter addObserver:self selector:@selector(textFieldTextDidChangeOneCI:) name:UITextFieldTextDidChangeNotification object:searchTextField];
...
-(void)creatAroundTableView{
    aroundTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, (HEIGHT - NAVI_HEIGHT)/2+NAVI_HEIGHT, WIDTH, (HEIGHT - NAVI_HEIGHT)/2) style:UITableViewStylePlain];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 10) {
        ...
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@%@%@%@",model.province,model.city,model.district,model.address];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(returnAddress:)]) {
        [self.delegate returnAddress:model];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
#pragma mark - 创建地图
-(void)creatMapView{
    _mapView = [[MAMapView alloc]initWithFrame:FRAME(0, 0, WIDTH, (HEIGHT - NAVI_HEIGHT)/2)];
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response
{
    ...
    for (AMapPOI * p in response.pois) {
        ...
        HZQSearchModel * model = [[HZQSearchModel alloc]init];
        model.province = p.province;
        model.city = p.city;
        model.district = p.district;
        model.address = p.address;
        model.name = p.name;
        model.latitude = @(p.location.latitude).description;
        model.longitude = @(p.location.longitude).description;
        if (isRoundSearch) {
