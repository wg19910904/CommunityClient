 ...
 #pragma mark =================
    ...
    if (sheetType == SheetViewChooseTime) {
        ...
        self.timeTableView.chooseBlock = ^(NSInteger left,NSInteger right,NSString *str){
            ...
            NSInteger dateline = [NSString getDatelineOfDate:str formate:@"yyyy-MM-dd HH:mm"];
            str = [NSString stringWithFormat:@"%ld",dateline];
    ...
    if (sheetType == SheetViewChoosePayType) {
        ...
        self.dataSource = @[@{@"title":NSLocalizedString(@"支付宝支付", @"JHCreateOrderSheetView"),@"icon":@"payWay01"},
                            @{@"title":NSLocalizedString(@"微信支付", @"JHCreateOrderSheetView"),@"icon":@"payWay02"},
                            @{@"title":NSLocalizedString(@"余额支付", @"JHCreateOrderSheetView"),@"icon":@"payWay03"}];
    }
...
-(void)setDataSource:(NSArray *)dataSource{
    ...
    switch (self.sheetType) {
        ...
        {
            self.timeTableView.isStartSelector = YES;
    ...
    if (self.sheetType == SheetViewChoosePayType || self.sheetType == SheetViewChooseMoneyPayType) {
        ...
        if (self.sheetType == SheetViewChoosePayWay ) {
            ...
            if (indexPath.row == self.dataSource.count-1) {
                ...
                str = [NSString stringWithFormat:@"%@%@(%@%@%@)",dic[@"amount"],NSLocalizedString(@"元红包", @"JHCreateOrderSheetView"),NSLocalizedString(@"满", @"JHCreateOrderSheetView"),dic[@"min_amount"],NSLocalizedString(@"可用", @"JHCreateOrderSheetView")];
            }
...
//                NSDictionary *dic = self.dataSource[indexPath.row];
                str = [NSString stringWithFormat:@"%@%@(%@%@%@)",dic[@"coupon_amount"],NSLocalizedString(@"元优惠劵", @"JHCreateOrderSheetView"),NSLocalizedString(@"满", @"JHCreateOrderSheetView"),dic[@"order_amount"],NSLocalizedString(@"可用", @"JHCreateOrderSheetView")];
            }
        }else{
            str = self.dataSource[indexPath.row];
        }
...
-(UIView *)footerView{
    if (_footerView==nil) {
        ...
        [payBtn setTitle:NSLocalizedString(@"去支付", @"JHCreateOrderSheetView") forState:UIControlStateNormal];
...
-(CreateOrderChooseTimeView *)timeTableView{
    ...
    __weak typeof (self)weakSelf = self;
    [_timeTableView setHiddenBlock:^{
        [weakSelf sheetHidden];
    }];
