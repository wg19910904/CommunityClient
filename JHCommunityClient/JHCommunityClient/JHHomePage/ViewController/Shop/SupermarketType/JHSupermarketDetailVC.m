...
@interface JHSupermarketDetailVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation JHSupermarketDetailVC
{
    NSString *phone_num;
}
- (void)viewDidLoad {
...
- (void)createMainTableView
{
    if (!_mainTableView) {
        //初始化表视图
        _mainTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT - 100)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1) {
        ...
        if (indexPath.row == 1) {
            ...
            contentLabel.textColor = [UIColor colorWithRed:250/255.0 green:68/255.0 blue:39/255.0 alpha:1.0];
    ...
    if (indexPath.section == 3) {
        if (indexPath.row == 0) {
            ...
            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 5, WIDTH- 60, 30)];
            titleLabel.text = [NSString stringWithFormat:NSLocalizedString(@"营业时间:周一至周日 %@ - %@", nil),_dataDic[@"yy_stime"],_dataDic[@"yy_ltime"]];
            titleLabel.textColor = DETAIL_TEXTCOLOR;
            titleLabel.font = [UIFont systemFontOfSize:13];
            cell.bounds = CGRectMake(0, 0, WIDTH, CGRectGetHeight(titleLabel.frame));
            [cell addSubview:titleLabel];
...
- (UIView *)createSection_0
{
    ...
    numlabel.text = [NSLocalizedString(@"月售 ", nil)  stringByAppendingString:_dataDic[@"orders"]];
