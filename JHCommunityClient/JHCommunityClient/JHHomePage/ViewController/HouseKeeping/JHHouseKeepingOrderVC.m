...
#import "JHHouseKeepingOrderDetailVC.h"
...
@interface JHHouseKeepingOrderVC ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UITextViewDelegate,AVAudioPlayerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UIScrollViewDelegate,JHCreateOrderSheetViewDelegate>
{
...
@implementation JHHouseKeepingOrderVC
...
- (void)handleData
{
    ...
    if(self.index == 1){
        _assignPerson.text = NSLocalizedString(@"选择阿姨,否则系统随机指定", nil);
    }else{
        _assignPerson.text = self.ayiName;
    }
}
#pragma mark=====创建录音按钮========
...
- (void)submitOrderBnt
{
    ...
    else
    {
        ...
        NSLog(@"%@",dic);
        SHOW_HUD
        [HttpTool postWithAPI:@"client/house/order/create" params:dic dataDic:dataDic success:^(id json) {
            NSLog(@"json%@",json);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0){
        ...
        UILabel *nameLabel = [[UILabel alloc] initWithFrame:FRAME(70, 20, WIDTH - 100, 15)];
        nameLabel.text = self.name;
        nameLabel.font = FONT(14);
        nameLabel.textColor = [UIColor blackColor];
        [cell.contentView addSubview:nameLabel];

        UILabel *areaLabel = [[UILabel alloc] initWithFrame:FRAME(70, 50, WIDTH - 100, 15)];
        ...
        switch (indexPath.row) {
            ...
            default:
            {
                UITableViewCell *cell = [[UITableViewCell alloc] init];
                UIImageView *iconImg = [[UIImageView alloc] initWithFrame:FRAME(10, 11.5, 15, 15)];
                iconImg.image = IMAGE(@"hongbao_img");
                [cell.contentView addSubview:iconImg];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0)
    {
        JHHouseKeepingOrderDetailVC *detail = [[JHHouseKeepingOrderDetailVC alloc] init];
    ...
    else if(indexPath.section == 1)
    {
        ...
        {
            JHHouseKeepingAssignPersonVC *assignPerson = [[JHHouseKeepingAssignPersonVC alloc] init];
...
-(void)getHongBaoArr:(NSString *)cate_id{
    SHOW_HUD
    [HttpTool postWithAPI:@"client/house/order/preinfo" withParams:@{@"cate_id":cate_id} success:^(NSDictionary* json) {
        NSLog(@"家政红包 =======  %@",json);
        HIDE_HUD
        if (ISPostSuccess) {
