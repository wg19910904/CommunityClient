...
#import "JHHouseKeepingCateCell.h"
...
@interface JHHouseKeepingSubCateVc ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHHouseKeepingSubCateVc
...
- (void)createTableView
{
    ...
    if(_rightTabelView == nil)
    {
        ...
        _rightTabelView.backgroundColor = [UIColor colorWithRed:247/255.0 green:247/255.0 blue:247/255.0 alpha:1.0f];
        _rightTabelView.tableFooterView = [[UIView alloc] init];
        UIView *thread = [[UIView alloc] initWithFrame:FRAME(0, 0, WIDTH, 0.5)];
        thread.backgroundColor = LINE_COLOR;
        [_rightTabelView addSubview:thread];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(tableView == _leftTabelView) {
        ...
        JHHouseKeepingCateCell *cell = [_leftTabelView dequeueReusableCellWithIdentifier:identifier];
        if(cell == nil){
            cell = [[JHHouseKeepingCateCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        }
        if(indexPath.row == 0){
            ...
            HouseKeepingHomeCateModel *cateModel = _leftDataArray[indexPath.row - 1];
...
{
    if(tableView == _leftTabelView){
        JHHouseKeepingCateCell *cell = [_leftTabelView cellForRowAtIndexPath:indexPath];
        if(indexPath.row == 0){
...
- (void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(tableView == _leftTabelView){
        JHHouseKeepingCateCell *cell = [tableView cellForRowAtIndexPath:indexPath];
