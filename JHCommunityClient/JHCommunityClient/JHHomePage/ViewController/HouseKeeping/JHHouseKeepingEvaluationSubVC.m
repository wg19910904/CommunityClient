...
#import "DSToast.h"
@interface JHHouseKeepingEvaluationSubVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHHouseKeepingEvaluationSubVC
...
- (void)loadNewData
{
    ...
    [HttpTool postWithAPI:@"client/house/staffcomment" withParams:_dic success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"])
        {
            ...
            {
                HoseKeepingCommentFrameModel *frameModel = [[HoseKeepingCommentFrameModel alloc] init];
                HoseKeepingCommentModel *commentModel = [[HoseKeepingCommentModel alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JHHouseKeepingEvalutionCell *cell = [_tableView dequeueReusableCellWithIdentifier:identifier];
    if(cell == nil)
    {
        cell = [[JHHouseKeepingEvalutionCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    cell.frameModel = _dataArray[indexPath.row];
    if(cell.starView == nil)
    {
        cell.starView = [StarView addEvaluateViewWithStarNO:[cell.frameModel.commentModel.score floatValue] withStarSize:10 withBackViewFrame:FRAME(10, 20 + cell.evalutionLabel.frame.size.height, 80, 10)];
    }
    [cell.backView addSubview:cell.starView];
    return cell;
}
