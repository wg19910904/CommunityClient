...
#import "HouseKeepingMapPersonModel.h"
#import <AMapFoundationKit/AMapFoundationKit.h>

@interface JHHouseKeepingMapVC ()<MAMapViewDelegate,AMapSearchDelegate,XHMapViewDelegate>
{
    XHMapView *_map;
    ...
    NSMutableArray *_dataArray;//存放家政人员的信息
...
@implementation JHHouseKeepingMapVC
...
-(void)creatBubble{
    if (_imageV_bubble == nil) {
        ...
        _label_num.frame = FRAME(5, 0, 100, 45);
...
- (void)locationBnt
{
//    if((_map.centerCoordinate.latitude != _userLocation.coordinate.latitude) &&(_map.centerCoordinate.longitude != _userLocation.coordinate.longitude)){
//        _map.centerCoordinate = _userLocation.coordinate;
//    }
...
- (void)postWithleftLat:(float)leftLat leftLng:(float)leftLng rightLat:(float)rightLat rightLng:(float)rightLng
{
    ...
    NSLog(@"%@",dic);
    [HttpTool postWithAPI:@"client/house/map" withParams:dic success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"])
        {
            ...
            if(count.length == 0)
            {
                _label_num.text = NSLocalizedString(@"附近无家政阿姨", nil);
            }
            else
            {

                _label_num.text = [NSString stringWithFormat:NSLocalizedString(@"附近有%@名家政阿姨", nil),count];

            }
            for(NSDictionary *dic in items)
            {
                HouseKeepingMapPersonModel *personModel = [[HouseKeepingMapPersonModel alloc] init];
...
- (void)addAnnotion
{
    ...
    if(_dataArray.count != 0)
    {
        ...
        for(int i = 0 ; i <_dataArray.count; i ++)
        {
            ...
            HouseKeepingMapPersonModel *model = _dataArray[i];
            float lat = model.lat ;
            float lng = model.lng ;
            personPoint.coordinate = CLLocationCoordinate2DMake(lat,lng);
            [_personAnnotionArray addObject:personPoint];
            [_map addAnnotation:personPoint.coordinate  title:@"" imgStr:@"ayi" selected:NO];
        }
...
- (MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id<MAAnnotation>)annotation
{
    if ([annotation isKindOfClass:[MAPointAnnotation class]])
    {
        ...
        annotationView.image = [UIImage imageNamed:@"ayi"];
