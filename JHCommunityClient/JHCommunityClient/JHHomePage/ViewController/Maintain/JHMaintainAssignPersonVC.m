...
#import "DSToast.h"
@interface JHMaintainAssignPersonVC ()<UITableViewDataSource,UITableViewDelegate>
{
    ...
    MaintainAssginPersonCell *_lastCell;
    DSToast *toast;
...
@implementation JHMaintainAssignPersonVC
...
//    MaintainAssginPersonCell * cell = (MaintainAssginPersonCell *)[_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:[self.staff_id integerValue] inSection:0]];
    ...
    _lastCell.selectImg.image = IMAGE(@"selectDefault");
     _shiFuTag = self.staff_id ? self.staff_id : [[NSUserDefaults standardUserDefaults] objectForKey:@"shiFuTag"];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"shiFuTag"];
...
- (void)loadNewData
{
    ...
    [HttpTool postWithAPI:@"client/weixiu/items" withParams:dic success:^(id json) {
        NSLog(@"json%@",json);
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MaintainAssginPersonCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    MaintainListModel *model = _dataArray[indexPath.row];
    if(cell == nil)
    {
        cell = [[MaintainAssginPersonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    if([_shiFuTag integerValue] == [model.staff_id integerValue])
    {
...
{
    MaintainAssginPersonCell *cell = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:indexPath.row inSection:0]];
    MaintainListModel *model = _dataArray[indexPath.row];
    if(_lastCell != nil)
    {
        _lastCell.selectImg.image = IMAGE(@"selectDefault");
    ...
    [[NSUserDefaults standardUserDefaults] setObject:model.staff_id forKey:@"shiFuTag"];
