...
#import "DSToast.h"
@interface JHMaintainListVc ()<UITableViewDataSource,UITableViewDelegate>
{
    UITableView *_tableView;
    NSMutableArray *_dataArray;
    MaintainListBnt *_cateBnt;
    MaintainListBnt *_distantBnt;
    BOOL _isCateSelected;
    BOOL _isDistantSelected;
    JHMaintainSubCateVc *_subCateVc;
    JHMaintainSubSortVc *_distantVc;
    NSInteger _page;
...
@implementation JHMaintainListVc
...
- (void)viewDidLoad {
    ...
    _subCateVc = [[JHMaintainSubCateVc alloc] init];
    _distantVc = [[JHMaintainSubSortVc alloc] init];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateButtonStatus:) name:@"updateMaintainList" object:nil];
...
- (void)createCateAndSortView{
    ...
    _cateBnt = [[MaintainListBnt alloc] initWithFrame:FRAME(0.25*WIDTH-50, 2.5, 100, 40)];
    ...
    _distantBnt = [[MaintainListBnt alloc] initWithFrame:FRAME(WIDTH*0.75-50, 2.5, 100, 40)];
    ...
    middleLine.frame = FRAME((WIDTH - 0.5) / 2, 5, 0.5, 30);
...
- (void)loadNewData
{
    ...
    [HttpTool postWithAPI:@"client/weixiu/items" withParams:_dic success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"])
        {
            ...
            {
                MaintainListModel *model = [[MaintainListModel alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MaintainListCell *cell = [_tableView dequeueReusableCellWithIdentifier:identifier];
    if(cell == nil)
    {
        cell = [[MaintainListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
...
{
    JHMaintainDetailVC *detail = [[JHMaintainDetailVC alloc] init];
