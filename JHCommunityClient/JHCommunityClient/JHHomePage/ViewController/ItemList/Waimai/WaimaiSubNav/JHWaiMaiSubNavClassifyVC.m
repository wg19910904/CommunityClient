...
@interface JHWaiMaiSubNavClassifyVC ()<UITableViewDelegate, UITableViewDataSource>
@property(nonatomic,strong)NSMutableDictionary *selectRowDic; //@{leftRow:x,rightRow:y}
@end

@implementation JHWaiMaiSubNavClassifyVC
{
...
- (void)loadNewData
{
    waimaiShopCateArray = [JHShareModel shareModel].waimaiShopCate_array;
    if (!waimaiShopCateArray) {
        SHOW_HUD
        //请求数据
        [HttpTool postWithAPI:@"client/waimai/shop/cate"
                   withParams:@{}
                      success:^(id json) {
                          NSLog(@"client/waimai/shop/cate---%@",json);
                          waimaiShopCateArray = json[@"data"][@"items"];
                          [JHShareModel shareModel].waimaiShopCate_array = waimaiShopCateArray;
                          [self createLeftTable];
                          [self createRightTable];
                           HIDE_HUD
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (tableView.tag) {
        case 100:  //左
            return waimaiShopCateArray.count + 1;
            break;
        case 200:  //右
    ...
    switch (tableView.tag) {
        case 100:
        {
            ...
            if (row == 0) {
                ...
                [iconImg sd_setImageWithURL:[NSURL URLWithString:[IMAGEADDRESS stringByAppendingString: waimaiShopCateArray[row - 1][@"icon"]]] placeholderImage:IMAGE(@"shop_cate_default")];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _leftTable) {
        if (row == 0) {
            ...
            _refreshBtnTitleBlock(NSLocalizedString(@"全部分类", nil));
            [self touch_BackView];
        }else{
