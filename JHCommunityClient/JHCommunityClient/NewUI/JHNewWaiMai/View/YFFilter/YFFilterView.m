...
#import "WaiMaiCateModel.h"
#import "WaiMaiAreaModel.h"
...
-(void)configUIWith:(NSArray *)titleArr{
    ...
    for (NSInteger i=0; i<titleArr.count; i++) {
        ...
        [btn setImage:IMAGE(@"icon-arrowD") forState:UIControlStateNormal];
        [btn setImage:IMAGE(@"icon-arrowUp") forState:UIControlStateSelected];
...
-(void)startAnimation{
    ...
    if (self.threeOrTwoFilter) {// 三个筛选条件
        if (self.selectedBtn.tag - btn_tag == 0) {
            ...
            height = self.firstArr.count > 8 ? 44 * 8: 44 * 5;
            ...
            self.sortTableView.height = 0;
            height = self.secondArr.count > 8 ? 44 * 8: 44 * 5;
            [UIView animateWithDuration:0.25 animations:^{
...
-(void)setFirstArr:(NSArray *)firstArr{
    ...
    _cateTableView.chooseBlock = ^(NSInteger left,NSInteger right){
        NSString *title ;
        WaiMaiCateModel *cate = firstArr[left];
        if (right >= 0) {
...
-(FilterTableView *)areaTableView{
    if (_areaTableView==nil) {
        ...
        _areaTableView.filterType = FilterOneTable;
        _areaTableView.kindType = 2;
    }
...
-(void)getData{
    ...
    [HttpTool postWithAPI:api withParams:@{} success:^(id json) {
        ...
        if ([json[@"error"] isEqualToString:@"0"]) {
            ...
            NSArray *cateArr = [WaiMaiCateModel mj_objectArrayWithKeyValuesArray:json[@"data"][@"items"]];
            for (int i = 0; i < [json[@"data"][@"items"]count]; i++) {
                NSLog(@"名字是:%@\n",json[@"data"][@"items"][i][@"title"]);
            }
            NSArray *areaArr = json[@"data"][@"filter"];
            NSArray *orderArr = json[@"data"][@"order"];
            weakSelf.firstArr = cateArr;
            weakSelf.secondArr = orderArr;
            weakSelf.thirdArr = areaArr;
...
//-(void)setFirstSelectedType:(NSString *)firstSelectedType{
//    if (firstSelectedType.length == 0 || !firstSelectedType) {
//        return;
//    }
//    YFTypeBtn *btn = [self viewWithTag:btn_tag];
//    self.cateTableView.firstShowSelectedIndex = 1;
//    [btn setTitle:firstSelectedType forState:UIControlStateNormal];
//    
//}
...
-(void)dealwithFirstSelectedType{
    ...
    NSString *title = @"";
    for (WaiMaiCateModel *cate in self.firstArr) {
        if ([cate.cate_id isEqualToString:_firstSelectedType]) {
            ...
            if (cate.childrens.count == 0) {
                ...
                for (WaiMaiCateModel *sub in cate.childrens) {
    ...
    WaiMaiCateModel *cate = self.firstArr[leftIndex];
    YFTypeBtn *btn = [self viewWithTag:btn_tag];
