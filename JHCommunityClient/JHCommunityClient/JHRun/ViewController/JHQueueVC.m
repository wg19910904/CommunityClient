...
@interface JHQueueVC ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate,UITextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,AVAudioPlayerDelegate>
{
    ...
    NSString * _paidui_start_price;//起始价
    NSString * voice_time;
...
-(void)initData{
    ...
    [HttpTool postWithAPI:@"client/paotui/gongshi" withParams:@{@"type":@"paidui"} success:^(id json) {
        NSLog(@"%@",json);
        if ([json[@"error"]intValue] == 0) {
            _paidui_start_price = json[@"data"][@"config"][@"paidui_start_price"];
            min_num = [_paidui_start_price floatValue];
            money = min_num;
    ...
    self.navigationItem.title = NSLocalizedString(@"代排队", nil);
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        cell.start_price = _paidui_start_price;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else{
...
-(NSInteger)changeWithDay:(NSString * )day withHour:(NSString *)hour withMinute:(NSString *)minute{
    ...
    NSDate * dateNew = [NSDate dateWithTimeIntervalSince1970:dateInterval];
    NSString * string_time = [dateForMatter stringFromDate:dateNew];
    string_time = [string_time substringToIndex:10];
    NSString * finalDateString = [NSString stringWithFormat:@"%@ %@:%@:00",string_time,hour,minute];
    NSDate * dateLast = [dateForMatter dateFromString:finalDateString];
    NSInteger finalInteval = [dateLast timeIntervalSince1970];
    return finalInteval;
}
...
-(void)sliderValueChanged:(UISlider *)sender{
    ...
    money =[newValue floatValue] + min_num;
...
#pragma mark - 创建底部弹出的选择照相还是从相册选择的view
...
#pragma mark - 这是imagePickerController的代理方法
...
-(void)clickToTrue{
    ...
    SHOW_HUD
    [HttpTool postWithAPI:@"client/paotui/paidui" params:dic dataDic:dataDic success:^(id json) {
        HIDE_HUD
        NSLog(@"%@",json[@"error"]);
