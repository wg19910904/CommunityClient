...
@interface JHPetVC ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate,UITextViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,AVAudioPlayerDelegate>
{
...
-(void)initData{
    ...
    self.navigationItem.title = NSLocalizedString(@"宠物照顾", nil);
    
    [HttpTool postWithAPI:@"client/paotui/gongshi" withParams:@{@"type":@"chongwu"} success:^(id json) {
        NSLog(@"%@",json);
        if ([json[@"error"]intValue] == 0) {
            _chongwu_start_price = json[@"data"][@"config"][@"chongwu_start_price"];
            min_num = [_chongwu_start_price floatValue];
            money = min_num;
...
//        cell.label1.text = [NSString stringWithFormat:@"%.2f",min_num];
        ...
        cell.start_price = _chongwu_start_price;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else{
...
-(NSInteger)changeWithDay:(NSString * )day withHour:(NSString *)hour withMinute:(NSString *)minute{
    ...
    if ([day isEqualToString:NSLocalizedString(@"今天", nil)]) {
        ...
        dateInteval +=3600*24*2;
    }
    NSDateFormatter * formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate * dateNew = [NSDate dateWithTimeIntervalSince1970:dateInteval];
    NSString * str_time = [formatter stringFromDate:dateNew];
    str_time = [str_time substringToIndex:10];
    NSString * str_lastTime = [NSString stringWithFormat:@"%@ %@:%@:00",str_time,hour,minute];
    NSDate * dateLast = [formatter dateFromString:str_lastTime];
    NSInteger finalIntaval = [dateLast timeIntervalSince1970];
    return finalIntaval;
...
#pragma mark - 这是pickerImageController的代理方法
...
-(void)clickToTrue{
    ...
    SHOW_HUD
    [HttpTool postWithAPI:@"client/paotui/chongwu" params:dic dataDic:dataDic success:^(id json) {
        HIDE_HUD
