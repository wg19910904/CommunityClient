...
- (UITableView *)addressTable{
    ...
    if (_addressTable == nil) {
        _addressTable = ({
            UITableView *tableView = [[UITableView alloc] initWithFrame:FRAME(0, NAVI_HEIGHT, WIDTH, HEIGHT-NAVI_HEIGHT-60)
...
- (void)clickDeleteBtn:(UIButton *)sender{
    ...
    NSIndexPath *indexpath = [self.addressTable indexPathForCell:cell];
    //获取模型
    [JHShowAlert showAlertWithTitle:NSLocalizedString(@"确定删除该地址", nil) withMessage:@"" withBtn_cancel:@"否" withBtn_sure:@"是" withCancelBlock:nil withSureBlock:^{
...
-(void)postToGetData{
    if (_shop_id) {
        ...
        [JHWaimaiMineViewModel postToGetShopAddressWithDic:@{@"shop_id":self.shop_id} block:^(JHWaimaiMineAddressListModel * model, NSString *error) {
            ...
            [_addressTable.mj_footer endRefreshing];
        }];
    }else{
        SHOW_HUD
        BOOL is_paotui_chooseAddr = self.is_paotui && self.selectorBlock;
