...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = HEX(@"f8f8f8", 1);
...
//    }
        ...
        cell.contentF.clearButtonMode =UITextFieldViewModeWhileEditing;
...
- (UITableView *)addressTable{
    ...
    if (_addressTable == nil) {
        _addressTable = ({
            UITableView *tableView = [[UITableView alloc] initWithFrame:FRAME(0, NAVI_HEIGHT, WIDTH, HEIGHT-NAVI_HEIGHT -110)
                                                                  style:(UITableViewStylePlain)];
            tableView.delegate = self;
            tableView.dataSource = self;
            tableView.backgroundColor = HEX(@"f8f8f8", 1);
...
-(void)creatUI{
   ...
   UIButton* _saveBtn = [[UIButton alloc] initWithFrame:FRAME(15, HEIGHT-BOTTOM_SAFE_HEIGHT- 110, WIDTH - 30, 40)];
    ...
    [_saveBtn addTarget:self action:@selector(addAddress) forControlEvents:(UIControlEventTouchUpInside)];
    _saveBtn.layer.cornerRadius = 2;
    [self.view addSubview:_saveBtn];
    
   UIButton* _deleteBtn = [[UIButton alloc] initWithFrame:FRAME(15, HEIGHT-BOTTOM_SAFE_HEIGHT- 50, WIDTH - 30, 40)];
    ...
    [_deleteBtn addTarget:self action:@selector(removeAddress) forControlEvents:(UIControlEventTouchUpInside)];
...
-(void)removeAddress{
    ...
    [JHShowAlert showAlertWithTitle:(NSString *)nil withMessage:(NSString *)@"确定删除该地址?" withBtn_cancel:@"取消" withBtn_sure:@"确定" withCancelBlock:nil withSureBlock:^{
        SHOW_HUD
