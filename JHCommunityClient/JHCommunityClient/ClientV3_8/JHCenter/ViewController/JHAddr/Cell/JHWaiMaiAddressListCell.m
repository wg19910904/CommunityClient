...
-(UIControl *)bgView{
    if (!_bgView) {
        ...
        label.textColor = THEME_COLOR_Alpha(1);
        label.font = FONT(15);
...
- (void)setupView{
    ...
    [_editBtn setImageEdgeInsets:UIEdgeInsetsMake(5, -5, 5, 5)];
    [_editBtn setTitle:@"编辑" forState:0];
    _editBtn.titleLabel.font = FONT(13);
    [_editBtn setTitleColor:HEX(@"333333", 1) forState:0];
    ...
    [_deleteBtn setTitle:@"删除" forState:0];
     [_deleteBtn setTitleColor:HEX(@"333333", 1) forState:0];
    [_deleteBtn setImageEdgeInsets:UIEdgeInsetsMake(5, -5, 5, 0)];
...
-(void)reloadCellWithModel:(JHWaimaiMineAddressListDetailModel *)model is_choose_paotui:(BOOL)is_choose_paotui{
    ...
    tagL.layer.borderColor =HEX(model.typeColor, 1.0).CGColor;
    tagL.textColor =HEX(model.typeColor, 1.0);
    tagL.layer.borderWidth = 1;
