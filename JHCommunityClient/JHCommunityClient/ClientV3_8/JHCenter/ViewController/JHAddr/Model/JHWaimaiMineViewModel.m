...
+(void)postToGetUserInfoWithBlock:(void(^)(NSString *error))myBlock{
    [HttpTool postWithAPI:@"client/v3/member/member/info" withParams:@{} success:^(id json) {
        ...
        NSString *err = nil;
        if (ISPostSuccess) {
            MemberInfoModel *model = [MemberInfoModel shareModel];
...
//    [[JHMallDataBaseTool shareInStance]deleteCartAllTable];
...
+(void)postToGetMyAddressWithDic:(NSDictionary *)dic is_paotui:(BOOL)is_paotui
    ...
    NSString *api = is_paotui ? @"client/v3/paotui/addr/items" : @"client/member/addr/index";
    if (is_paotui) {
...
//    [HttpTool postWithAPI:@"client/member/log" withParams:dic success:^(id json) {
//        NSLog(@"%@",json);
//        NSString *err = nil;
