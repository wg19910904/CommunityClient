...
@interface JHWaiMaiOrderVC ()<YFIndexIndicatorViewDelegate,YFPageViewControllerDelegate>
...
-(void)viewDidLoad{
    ...
    [super viewDidLoad];
    [self setUpView];
    self.navigationItem.title =  NSLocalizedString(@"外卖", NSStringFromClass([self class]));
...
-(void)setUpView{
    ...
    for (int i=0; i<arr.count; i++) {
        JHWaiMaiOrderListVC *vc = [JHWaiMaiOrderListVC new];
        vc.yf_base_index = i;
        vc.superVC = self;
