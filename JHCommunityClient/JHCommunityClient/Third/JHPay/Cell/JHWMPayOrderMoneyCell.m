...
-(void)relaodCellWith:(NSString *)order_money{
    self.switchBtn.userInteractionEnabled =  [[MemberInfoModel shareModel].money floatValue] > 0;
    if ([[MemberInfoModel shareModel].money floatValue] <= 0) {
...
-(void)getAttrStrWith:(NSString *)amount{
    ...
    if (self.switchBtn.isOn) {
        if ([[MemberInfoModel shareModel].money floatValue] >= [amount floatValue]) {
            amount = @"0";
        }else{
            float discount = [amount floatValue] - [[MemberInfoModel shareModel].money floatValue]  ;
    ...
    NSString *money =  [NSLocalizedString(@"¥", NSStringFromClass([self class])) stringByAppendingString:[MemberInfoModel shareModel].money];
